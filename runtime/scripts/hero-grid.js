...
(function () {
  ...
  /* ═══════════════════════════════════════════
     ...
     ═══════════════════════════════════════════ */
  /* Hub card index → grid column. Adapts to 3 or 4 cards:
     4 cards: [0, 1, 3, 4] (col 2 has no bottom)
     3 cards: [0, 2, 4]    (evenly distributed) */
  var HUB_COL_MAP = hubCards.length >= 4 ? [0, 1, 3, 4] : [0, 2, 4];
  var allImages = [];
  var colGroups = [[], [], [], [], []];
  var bottomImages = [];
  ...
  function buildImageModel() {
    ...
    /* Bottom-row hub cards (3 or 4 items). Appended as the last row of their
       column so the existing grid stacking math keeps working unchanged. */
    hubCards.forEach(function (card, i) {
      if (i >= HUB_COL_MAP.length) return; // safety: skip cards beyond map
      var colIdx = HUB_COL_MAP[i];
      var img = card.querySelector('.hhub-card-img');
      /* Fallback when .hhub-card-img is absent (gradient-fallback variant):
         use the card's .hhub-card-media dimensions or sensible defaults. */
      var w, h;
      if (img) {
        w = parseInt(img.getAttribute('width'), 10) || 584;
        h = parseInt(img.getAttribute('height'), 10) || 584;
      } else {
        var media = card.querySelector('.hhub-card-media');
        w = media ? media.offsetWidth || 584 : 584;
        h = media ? media.offsetHeight || 329 : 329;
        img = media || card; // use media container as the "img" reference
      }
      var data = {
