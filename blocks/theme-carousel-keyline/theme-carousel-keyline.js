/**
 * Theme Carousel Keyline Block — Variant E: Layered Green Keyline
 * Authoring rows:
 *   Row 1: section heading (h2)
 *   Row 2: description text
 *   Row 3: quick links (multiple <a> elements)
 *   Rows 4+: tiles — Cell 0: img/picture, Cell 1: tag text, Cell 2: h3, Cell 3: cta link
 */
...
export default function decorate(block) {
  ...
  wrapper.className = `theme-carousel-keyline-wrapper ${surfaceClass}`;
  ...
  tileRows.forEach((row) => {
    ...
    tile.className = 'tile-e';
    tile.href = ctaHref;

    // Photo
    const photoDiv = document.createElement('div');
    photoDiv.className = 'tile-e-photo';

    if (tagText) {
      const tag = document.createElement('span');
      tag.className = 'tile-e-tag';
    ...
    if (imgEl) {
      ...
      empty.className = 'tile-e-photo-empty';
      photoDiv.appendChild(empty);
    }
