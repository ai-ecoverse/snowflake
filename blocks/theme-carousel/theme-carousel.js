/**
 * Theme Carousel Block — Variant A: Editorial Wide-Card (16/11)
 * Authoring rows:
...
export default function decorate(block) {
  ...
  // Collect surface class from block (surface-white / surface-dark)
  const surfaceClass = [...block.classList].find((c) => c.startsWith('surface-')) || 'surface-white';
  ...
  wrapper.className = `theme-carousel-wrapper ${surfaceClass}`;
  ...
  tileRows.forEach((row) => {
    ...
    if (imgEl) {
      ...
      empty.className = 'tile-a-photo-empty';
    ...
    eyebrowEl.className = 'tile-a-eyebrow';
    eyebrowEl.textContent = eyebrow;
    band.appendChild(eyebrowEl);
