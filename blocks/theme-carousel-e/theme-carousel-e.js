/**
 * theme-carousel-e — Layered Green Keyline carousel (variant E), 3-up.
 *
 * Authoring rows:
 *   Row 1: heading text
 *   Row 2: description text
 *   Row 3: quick links — "Label|URL, Label|URL, ..."
 *   Row 4+: tile rows — cells: image | tag | headline | CTA text | URL
 *
...
export default async function decorate(block) {
  ...
  tileRows.forEach((tileRow) => {
    ...
    photo.className = 'tile-e-photo';
    if (img) photo.append(img.cloneNode(true));
    if (tag) {
      const tagEl = document.createElement('span');
      tagEl.className = 'tile-e-tag';
