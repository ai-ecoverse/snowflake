 ...
 *   2. h1 headline
 *   3. sub paragraph
 *   4. phone CTA — cell contains an <a href="tel:..."> element
 *   5. background image (picture or img)
 */
export default async function decorate(block) {
  ...
  // Clone the anchor from row 4 (phone CTA)
  const phoneAnchor = rows[3]?.querySelector('a');
  let phoneCTA = null;
  if (phoneAnchor) {
    phoneCTA = phoneAnchor.cloneNode(true);
    phoneCTA.className = 'phone-cta';
  }

  // Extract image src from row 5
  const imgEl = rows[4]?.querySelector('img');
  ...
  left.append(eyebrow, h1, sub);
  if (phoneCTA) left.append(phoneCTA);
